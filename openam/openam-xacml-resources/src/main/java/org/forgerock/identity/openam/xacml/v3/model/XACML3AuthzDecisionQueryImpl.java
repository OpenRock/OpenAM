 ...
 *@supported.all.api
 */
public class XACML3AuthzDecisionQueryImpl extends RequestImpl
        implements XACMLAuthzDecisionQuery, XACML3Constants {
     ...
     * Default constructor
     */
    public XACML3AuthzDecisionQueryImpl() {
        isMutable = true;
    }
     ...
     * @exception com.sun.identity.saml2.common.SAML2Exception if it could not process the Element
     */
    public XACML3AuthzDecisionQueryImpl(Element element) throws SAML2Exception {
     ...
     * @exception com.sun.identity.xacml.common.XACMLException if it could not process the XML string
     */
    public XACML3AuthzDecisionQueryImpl(String xml) throws SAML2Exception {
     ...
     * of this attribute.
     *
     * @exception com.sun.identity.xacml.common.XACMLException if the object is immutable
     ...
     * @exception com.sun.identity.xacml.common.XACMLException if conversion fails for any reason
     */
    ...
    protected void parseDOMElement(XACMLRequestInformation xacmlRequestInformation) throws SAML2Exception {
        //TODO: fix
        String value = null;
        if ( (xacmlRequestInformation.getAuthenticationContent() == null) ||
             (xacmlRequestInformation.getContent() == null) ) {
            XACMLSDKUtils.debug.error(
        ...
        // Adding [SAML4XACML] Authentication Pattern.
        if ( (element.getNodeName()==null) ||
             (!element.getNodeName().toLowerCase().contains(XACML_AUTHZ_QUERY.toLowerCase())) ||
             (element.getLocalName()==null) ||
             (!element.getLocalName().equalsIgnoreCase(XACMLConstants.REQUEST_ABSTRACT)) ) {
            XACMLSDKUtils.debug.error(
                    "XACMLAuthzDecisionQueryImpl.processElement(): "
                    + "invalid root element");
            throw new XACMLException( XACMLSDKUtils.xacmlResourceBundle.getString(
        ...
        **/
        ...
        request = factory.getInstance().createRequest(
                (Element)xacmlRequestInformation.getContent());
        ...
        System.out.println("ReturnContex:" + ((Element)xacmlRequestInformation.getContent()).getAttributeNS(
                XACMLConstants.XACML_SAMLP_NS_URI,
                XACMLConstants.RETURN_CONTEXT));
        System.out.println("InputContextOnly:" + ((Element)xacmlRequestInformation.getContent()).getAttributeNS(
                XACMLConstants.XACML_SAMLP_NS_URI,
                XACMLConstants.INPUT_CONTEXT_ONLY));
        String returnContextString = ((Element)xacmlRequestInformation.getContent()).getAttributeNS(
                XACMLConstants.XACML_SAMLP_NS_URI,
                XACMLConstants.RETURN_CONTEXT);
        ...
        String inputContextOnlyString = ((Element)xacmlRequestInformation.getContent()).getAttributeNS(
                XACMLConstants.XACML_SAMLP_NS_URI,
                XACMLConstants.INPUT_CONTEXT_ONLY);
        ...
        NamedNodeMap attrs = ((Element)xacmlRequestInformation.getContent()).getAttributes();

        //TODO: change the baseclass impl and call super.parse...

        //parse the attributes of base class RequestAbstract
        NamedNodeMap atts = ((Node)xacmlRequestInformation.getContent()).getAttributes();
        if (atts != null) {
	...
	NodeList nl = ((Element)xacmlRequestInformation.getContent()).getChildNodes();
        Node child;
        String childName;
