 ...
 * <p/>
 * If it request for Token Endpoint then the grant_type [authorization_code,password,client_credentials,refresh_token,urn:ietf:params:oauth:grant-type:saml2-bearer]
 */
public class OAuth2FlowFinder extends Finder {
    ...
    private volatile Map<String, Class<? extends AbstractFlow>> flowServerResources = new ConcurrentHashMap<String, Class<? extends AbstractFlow>>(6);
    ...
    public AbstractFlow create(Class<? extends AbstractFlow> targetClass, Request request, Response response) {
        ...
        if (targetClass != null) {
            try {
                ...
                OAuthProblemException.OAuthError.SERVER_ERROR.handle(request, e.getMessage()).pushException();
            ...
            if (type instanceof String) {
                ...
                if (targetClass == null) {
                    if (OAuth2.EndpointType.AUTHORIZATION_ENDPOINT.equals(endpointType)) {
                        targetClass = OAuthProblemException.OAuthError.UNSUPPORTED_RESPONSE_TYPE.handle(request,
                                "Type is not supported: " + type).pushException();
                    } else {
                        targetClass = OAuthProblemException.OAuthError.UNSUPPORTED_GRANT_TYPE.handle(request,
                                "Type is not supported: " + type).pushException();
                    }
                }
            } else {
                targetClass = OAuthProblemException.OAuthError.NOT_FOUND.handle(request, "Type is not set").pushException();
            }
        } else {
            targetClass = OAuthProblemException.OAuthError.NOT_FOUND.handle(request, "Type is not set").pushException();
        }
